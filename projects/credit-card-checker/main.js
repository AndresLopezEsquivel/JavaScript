...
const validateCred = creditCardArray => {
  // Checks if a credit card is valid based on Luhn algorithm
  // Return true if it's valid, false otherwise.
  let sum = 0;
  let creditCardArrayLength = creditCardArray.length;
  let lengthIsEven = isEven(creditCardArrayLength);
  let currentNumber = 1;

  for(let i = creditCardArrayLength - 1; i >= 0; i--)
  {
    currentNumber = 1;
    if(lengthIsEven)
    {
      currentNumber *= creditCardArray[i] * (isEven(i) ? 2 : 1);
    }
    else
    {
      currentNumber *= creditCardArray[i] * (isEven(i) ? 1 : 2);
    }
...
console.log("Testing invalid credit cards:");
